...
const _getYoutubeVideo = db.prepare('SELECT video_id, embeddable, found FROM youtube_video_cache WHERE key = ?');
const _setYoutubeVideo = db.prepare('INSERT OR REPLACE INTO youtube_video_cache (key, video_id, embeddable, found) VALUES (?, ?, ?, ?)');

export function getCachedYoutubeVideo(artist, title) {
    const row = _getYoutubeVideo.get(_youtubeKey(artist, title));
    if (!row) return null;
    return { videoId: row.video_id, embeddable: !!row.embeddable, found: !!row.found };
}

export function setCachedYoutubeVideo(artist, title, { videoId = null, embeddable = true, found = true } = {}) {
    _setYoutubeVideo.run(_youtubeKey(artist, title), videoId, embeddable ? 1 : 0, found ? 1 : 0);
}
...
const _getArtistChannel = db.prepare('SELECT channel_id, found FROM youtube_artist_channel WHERE key = ?');
...
export function getArtistChannel(artist) {
    ...
    return row ? { channelId: row.channel_id, found: !!row.found } : null;
}
