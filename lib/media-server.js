import { soapCall } from './upnp.js';
import xml2js from 'xml2js';

export default class MediaServer {
    constructor(device) {
        this.device = device;
        this.browseService = device.services?.find(s => s.serviceType.includes('ContentDirectory'));
    }

    _getText(node) {
        if (!node) return '';
        if (Array.isArray(node)) return this._getText(node[0]);
        if (typeof node === 'string') return node;
        if (typeof node === 'object') {
            if (node._ !== undefined) return node._;
            if (node.$ && Object.keys(node).length === 1) return '';
        }
        return String(node);
    }

    async browse(objectId = '0', timeoutMs = 30000) {
        if (!this.browseService) {
            throw new Error('ContentDirectory service not found on this device');
        }

        const doSoapBrowse = (sortCriteria) => soapCall(
            this.browseService.controlURL,
            this.browseService.serviceType,
            'Browse',
            {
                ObjectID: objectId,
                BrowseFlag: 'BrowseDirectChildren',
                Filter: '*',
                StartingIndex: 0,
                RequestedCount: 0,
                SortCriteria: sortCriteria
            },
            timeoutMs
        );

        let browseResponse;
        try {
            browseResponse = await doSoapBrowse('+upnp:originalTrackNumber');
        } catch (sortErr) {
            // Server doesn't support this sort criteria — retry unsorted
            browseResponse = await doSoapBrowse('');
        }

        const resultXml = browseResponse.Result;
        if (!resultXml) {
            return [];
        }

        let browseResult;

        // Check if the result is already a parsed object (some servers return JSON)
        if (typeof resultXml === 'object' && resultXml !== null) {
            browseResult = resultXml;
        } else if (typeof resultXml === 'string') {
            // Clean the XML to handle Windows 11 DLNA servers that may include BOM or other issues
            let cleanedXml = resultXml;

            // Remove BOM (Byte Order Mark) if present
            cleanedXml = cleanedXml.replace(/^\uFEFF/, '');
            // Trim any leading/trailing whitespace
            cleanedXml = cleanedXml.trim();

            // Check if it's actually JSON
            if (cleanedXml.startsWith('{') || cleanedXml.startsWith('[')) {
                try {
                    browseResult = JSON.parse(cleanedXml);
                } catch (jsonError) {
                    console.error('[BROWSE] JSON Parse Error:', jsonError.message);
                    throw new Error(`Failed to parse JSON browse response: ${jsonError.message}`);
                }
            } else {
                // It's XML - remove any non-XML characters before the first '<'
                const firstTag = cleanedXml.indexOf('<');
                if (firstTag > 0) {
                    cleanedXml = cleanedXml.substring(firstTag);
                }

                const parser = new xml2js.Parser({
                    explicitArray: false,
                    tagNameProcessors: [xml2js.processors.stripPrefix]
                });

                try {
                    browseResult = await parser.parseStringPromise(cleanedXml);
                } catch (parseError) {
                    console.error('[BROWSE] XML Parse Error:', parseError.message);
                    console.error('[BROWSE] First 200 chars of data:', cleanedXml.substring(0, 200));
                    throw new Error(`Failed to parse browse response: ${parseError.message}`);
                }
            }
        } else {
            throw new Error(`Unexpected result type: ${typeof resultXml}`);
        }


        const didl = browseResult.DIDL_Lite || browseResult['DIDL-Lite'] || browseResult;

        const items = [];

        // Handle Containers
        if (didl.container) {
            const containers = Array.isArray(didl.container) ? didl.container : [didl.container];
            containers.forEach(c => {
                items.push({
                    id: c.$.id || c.id,
                    type: 'container',
                    title: this._getText(c.title) || 'Unknown Folder',
                    artist: this._getText(c.artist),
                    albumArtist: this._getText(c.albumArtist || c.albumartist),
                    class: this._getText(c.class),
                    albumArtUrl: this._getText(c.albumArtURI || c['upnp:albumArtURI'])
                });
            });
        }

        // Handle Items
        if (didl.item) {
            const entries = Array.isArray(didl.item) ? didl.item : [didl.item];
            entries.forEach(item => {
                const res = Array.isArray(item.res) ? item.res[0] : item.res;
                items.push({
                    id: item.$.id || item.id,
                    type: 'item',
                    title: this._getText(item.title) || 'Unknown Item',
                    artist: this._getText(item.artist || item.creator),
                    albumArtist: this._getText(item.albumArtist || item.albumartist),
                    album: this._getText(item.album),
                    genre: this._getText(item.genre),
                    year: this._getText(item.date),
                    class: this._getText(item.class),
                    uri: this._getText(item.res),
                    albumArtUrl: this._getText(item.albumArtURI || item['upnp:albumArtURI']),
                    trackNumber: parseInt(this._getText(item.originalTrackNumber || item.trackNumber)) || 0,
                    discNumber: parseInt(this._getText(item.originalDiscNumber || item.discNumber)) || 1,
                    duration: (() => {
                        const resDuration = (res && res.$ && res.$.duration) ? res.$.duration : null;
                        const itemDuration = this._getText(item.duration || item['upnp:duration']);
                        return resDuration || itemDuration || null;
                    })(),
                    size: (res && res.$ && res.$.size) ? parseInt(res.$.size) : null,
                    resolution: (res && res.$ && res.$.resolution) ? res.$.resolution : null,
                    bitrate: (res && res.$ && res.$.bitrate) ? parseInt(res.$.bitrate) : null,
                    protocolInfo: (res && res.$ && res.$.protocolInfo) || 'http-get:*:audio/mpeg:*'
                });
            });
        }

        return items;
    }
    async browseRecursive(objectId, rootTitle = 'Home', concurrency = 6) {
        let allItems = [];
        const queue = [{ id: objectId, path: [{ id: objectId, title: rootTitle }], title: rootTitle }];
        const visited = new Set([objectId]);
        let errors = 0;

        while (queue.length > 0) {
            const batch = [];
            while (queue.length > 0 && batch.length < concurrency) {
                batch.push(queue.shift());
            }

            const results = await Promise.all(batch.map(async ({ id: currentId, path: currentPath, title: currentTitle }) => {
                try {
                    const items = await this.browse(currentId, 8000);
                    const newItems = [];
                    const newContainers = [];
                    for (const item of items) {
                        if (item.type === 'item') {
                            item._path = JSON.stringify(currentPath);
                            item.folderId = currentId;
                            item.folderTitle = currentTitle;
                            newItems.push(item);
                        } else if (item.type === 'container') {
                            newContainers.push({ id: item.id, path: [...currentPath, { id: item.id, title: item.title }], title: item.title });
                        }
                    }
                    return { items: newItems, containers: newContainers };
                } catch (err) {
                    errors++;
                    console.error(`Recursive browse error at ${currentId}:`, err.message);
                    return { items: [], containers: [] };
                }
            }));

            for (const { items, containers } of results) {
                allItems.push(...items);
                for (const c of containers) {
                    if (!visited.has(c.id)) {
                        visited.add(c.id);
                        queue.push(c);
                    }
                }
            }
        }
        this.lastCrawlErrors = errors;
        allItems.sort((a, b) => {
            // Sort by Album first (if multiple albums in one folder/tree)
            if (a.album !== b.album) {
                return (a.album || '').localeCompare(b.album || '');
            }
            // Sort by Disc
            if (a.discNumber !== b.discNumber) {
                return (a.discNumber || 1) - (b.discNumber || 1);
            }
            // Sort by Track
            if (a.trackNumber !== b.trackNumber) {
                return (a.trackNumber || 0) - (b.trackNumber || 0);
            }
            // Finally by Title
            return (a.title || '').localeCompare(b.title || '');
        });

        return allItems;
    }
}
