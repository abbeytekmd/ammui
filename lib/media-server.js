...
export default class MediaServer {
    ...
    async browseRecursive(objectId, rootTitle = 'Home') {
        let allItems = [];
        // Queue stores { id, path, title } - path is now an array of {id, title}
        const queue = [{ id: objectId, path: [{ id: objectId, title: rootTitle }], title: rootTitle }];
        const visited = new Set();

        while (queue.length > 0) {
            const { id: currentId, path: currentPath, title: currentTitle } = queue.shift();
            ...
            try {
                ...
                for (const item of items) {
                    if (item.type === 'item') {
                        // Attach folder info to item
                        item._path = JSON.stringify(currentPath); // Send as JSON string to preserve structure
                        item.folderId = currentId;
                        item.folderTitle = currentTitle;
                        allItems.push(item);
                    } else if (item.type === 'container') {
                        // Build new path array
                        const newPath = [...currentPath, { id: item.id, title: item.title }];
                        queue.push({ id: item.id, path: newPath, title: item.title });
                    }
                }
            } catch (err) {
