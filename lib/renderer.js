...
export default class Renderer {
    ...
    async getVolume() {
        ...
        throw new Error('Volume/RenderingControl service not found');
    }
    ...
    async setVolume(volume) {
        console.log(`[COMMAND] Sending SET_VOLUME command to ${this.device.friendlyName} (volume: ${volume})`);
        if (this.volumeService) {
    ...
    async getEQ() {
        if (!this.renderingControlService) return null;
        console.log(`[DEBUG] Fetching EQ for ${this.device.friendlyName} via soapCall...`);
        const [bassRes, trebleRes] = await Promise.all([
            soapCall(this.renderingControlService.controlURL, this.renderingControlService.serviceType, 'GetBass', { InstanceID: 0, Channel: 'Master' }),
            soapCall(this.renderingControlService.controlURL, this.renderingControlService.serviceType, 'GetTreble', { InstanceID: 0, Channel: 'Master' })
        ]);
        console.log(`[DEBUG] Raw EQ response for ${this.device.friendlyName}: bass=`, bassRes, `treble=`, trebleRes);
        return {
            bass: parseInt(bassRes.CurrentBass, 10) || 0,
            treble: parseInt(trebleRes.CurrentTreble, 10) || 0
        };
    }

    async setEQ(type, value) {
        console.log(`[COMMAND] Sending SET_EQ command to ${this.device.friendlyName} (${type}: ${value})`);
        if (!this.renderingControlService) return;
        const val = parseInt(value, 10);
        const action = type === 'bass' ? 'SetBass' : 'SetTreble';
        const param = type === 'bass' ? 'DesiredBass' : 'DesiredTreble';
        try {
            console.log(`[DEBUG] Setting ${type} to ${val} for ${this.device.friendlyName} via soapCall...`);
            await soapCall(this.renderingControlService.controlURL, this.renderingControlService.serviceType, action, {
                InstanceID: 0,
                Channel: 'Master',
                [param]: val
            });
            console.log(`[DEBUG] Successfully set ${type} to ${val}`);
        } catch (err) {
            console.error(`[DEBUG] Failed to set ${type} for ${this.device.friendlyName}:`, err.message);
        }
    }

    _getText(node) {
        if (!node) return '';
        if (Array.isArray(node)) return this._getText(node[0]);
        if (typeof node === 'string') return node;
        if (typeof node === 'object') {
            if (node._ !== undefined) return node._;
            // If it's an object but doesn't have _, it might be an empty tag with attributes
            if (node.$ && Object.keys(node).length === 1) return '';
        }
        return String(node);
    }
    ...
    async getPlaylist() {
        if (this.sonosDevice) {
            ...
            return await this._getAVTransportPlaylist();
        } else {
            throw new Error('No compatible playlist service found on this device');
        }
    }

    async _getAVTransportPlaylist() {
        if (!this.avTransportService) return [];
        ...
        if (!pos || !pos.TrackMetaData || pos.TrackMetaData === 'NOT_IMPLEMENTED' || pos.TrackMetaData.indexOf('<') === -1) {
            return [];
        }
        ...
        const result = await parser.parseStringPromise(pos.TrackMetaData);
        const didl = result['DIDL-Lite'] || result.DIDL_Lite || result;

        // Handle cases where item might be an array or nested differently
        let item = didl.item || didl;
        if (Array.isArray(item)) item = item[0];

        return [{
            id: pos.Track || 1,
            title: this._getText(item.title || item['dc:title']) || 'Unknown Title',
            artist: this._getText(item.artist || item['upnp:artist'] || item.creator),
            album: this._getText(item.album || item['upnp:album']),
            albumArtUrl: this._getText(item.albumArtURI || item['upnp:albumArtURI']),
            uri: pos.TrackURI || this._getText(item.res),
...
<DIDL-Lite xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/">
  <item id="f0002000s0" parentID="Q:0" restricted="1">
    ...
    ${track.albumArtUrl ? `<upnp:albumArtURI>${xmlEscape(track.albumArtUrl)}</upnp:albumArtURI>` : ''}
...
<DIDL-Lite xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/">
  <item id="0" parentID="-1" restricted="1">
    ...
    <res ${protocolInfo ? `protocolInfo="${xmlEscape(protocolInfo)}"` : ''} ${duration ? `duration="${xmlEscape(duration)}"` : ''}>${xmlEscape(uri)}</res>
            ...
            await soapCall(this.avTransportService.controlURL, this.avTransportService.serviceType, 'SetAVTransportURI', {
                InstanceID: 0,
                CurrentURI: uri,
                CurrentURIMetadata: metadata
            });
            return 1; // Simple devices only hold one track
        }
    ...
    async clearPlaylist() {
        if (this.sonosDevice) {
            ...
            // For simple renderers, "clear" is stop and ideally clearing URI (but stop is safer)
    ...
    async getIdArray() {
        if (this.sonosDevice) {
            ...
            const playlist = await this._getAVTransportPlaylist();
            return playlist.map(item => item.id);
        }
        throw new Error('Playlist service not found');
    }
    ...
    async play() {
        console.log(`[COMMAND] Sending PLAY command to ${this.device.friendlyName}`);
        if (this.sonosDevice) {
    ...
    async pause() {
        console.log(`[COMMAND] Sending PAUSE command to ${this.device.friendlyName}`);
        if (this.sonosDevice) {
    ...
    async stop() {
        console.log(`[COMMAND] Sending STOP command to ${this.device.friendlyName}`);
        if (this.sonosDevice) {
    ...
    async getCurrentStatus() {
        if (this.sonosDevice) {
            ...
            return {
                ...
                transportState: this._normalizeState(state),
            ...
            return {
                ...
                transportState: this._normalizeState(stateRes ? stateRes.Value || stateRes.TransportState : 'Stopped'),
                duration,
                relTime
            };
        } else if (this.avTransportService) {
            ...
            return {
                ...
                transportState: this._normalizeState(info ? info.CurrentTransportState : 'Stopped'),
    ...
    _normalizeState(state) {
        ...
        if (s === 'PAUSED_PLAYBACK' || s === 'PAUSED') return 'Paused';
        if (s === 'STOPPED') return 'Stopped';
        // Return original but capitalized first letter as fallback
    ...
    async seekId(id) {
        console.log(`[COMMAND] Sending SEEK_ID command to ${this.device.friendlyName} (id: ${id})`);
        if (this.sonosDevice) {
            try {
                const trackIndex = parseInt(id, 10);
                ...
                // Step 5: Start playback — retry on 701 (TransitionNotAvailable) which means
                // the device is still TRANSITIONING (buffering local content)
                let result;
                for (let attempt = 0; attempt < 4; attempt++) {
                    try {
                        result = await this.sonosDevice.play();
                        break;
                    } catch (playErr) {
                        if (attempt < 3 && playErr.message && playErr.message.includes('701')) {
                            console.warn(`[DEBUG] Sonos play() got 701 (TRANSITIONING), retrying in 500ms (attempt ${attempt + 1}/3)...`);
                            await new Promise(resolve => setTimeout(resolve, 500));
                        } else {
                            throw playErr;
                        }
            ...
            // For standard renderers without a playlist service, "seek" is effectively "play"
            // the current URI that was set.
            return await this.play();
        }
        throw new Error('Seek service not found');
    }
    async seekTime(seconds) {
        console.log(`[COMMAND] Sending SEEK_TIME command to ${this.device.friendlyName} (seconds: ${seconds})`);
        if (this.sonosDevice) {
            try {
