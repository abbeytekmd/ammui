...
export default class AirPlayRenderer {
    ...
    async play() {
        console.log(`[COMMAND] Sending PLAY command to ${this.device.friendlyName} (AirPlay)`);
    ...
    async pause() {
        console.log(`[COMMAND] Sending PAUSE command to ${this.device.friendlyName} (AirPlay)`);
        console.warn(`[AirPlay] Pause not fully supported via RAOP alone (requires tearing down stream). Stopping instead.`);
        this.stop();
    }

    async stop() {
        console.log(`[COMMAND] Sending STOP command to ${this.device.friendlyName} (AirPlay)`);
        this.isPlaying = false;
        try {
    ...
    async setVolume(volume) {
        console.log(`[COMMAND] Sending SET_VOLUME command to ${this.device.friendlyName} (AirPlay) (volume: ${volume})`);
        this.volume = volume;
        try {
            if (this.sender) {
    ...
    async seekTime(seconds) {
        console.log(`[COMMAND] Sending SEEK_TIME command to ${this.device.friendlyName} (AirPlay) (seconds: ${seconds})`);
