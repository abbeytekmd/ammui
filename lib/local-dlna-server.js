...
export function getLocalIp() {
  ...
  for (const devName in interfaces) {
    ...
    // Skip common VPN and virtual interfaces
    //    if (lowerName.includes('nord') || lowerName.includes('vpn') || lowerName.includes('tun') || lowerName.includes('tap') || lowerName.includes('lynx')) {
    //      continue;
    //    }
    ...
    for (const alias of iface) {
      if (alias.family === 'IPv4' && !alias.internal && alias.mac.localeCompare('00:00:00:00:00:00') !== 0) {
        ...
        //         preferred.push(alias.address);
        //       } else {
        //        others.push(alias.address);
        //       }
      }
    }
  }

  const result = preferred[0] || others[0] || '127.0.0.1';
  console.log(`[DEBUG] getLocalIp detected: ${result} (Preferred: ${preferred}, Others: ${others})`);
  return result;
}
...
export function setupLocalDlna(app, port) {
  ...
  // 1. Description XML
  app.get('/dlna/description.xml', (req, res) => {
...
<root xmlns="urn:schemas-upnp-org:device-1-0">
  ...
  app.get('/dlna/ContentDirectory.xml', (req, res) => {
...
<scpd xmlns="urn:schemas-upnp-org:service-1-0">
  ...
  app.get('/dlna/ConnectionManager.xml', (req, res) => {
    const xml = `<?xml version="1.0" encoding="utf-8"?>
<scpd xmlns="urn:schemas-upnp-org:service-1-0">
  <specVersion><major>1</major><minor>0</minor></specVersion>
  <actionList>
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
  ...
  app.post('/dlna/ConnectionManager/control', (req, res) => {
    const resp = `<?xml version="1.0" encoding="utf-8"?>
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
  <s:Body>
